...
var results = document.querySelector('.results'); //JSON data

var link = 'https://es.wikipedia.org/w/api.php?Special:MyLanguage&action=opensearch&format:json&search=';
submitInput.addEventListener('click', getInfo);

function getInfo(e) {
  e.preventDefault(); //Search link

  var searchValue = textInput.value;
  var toSearch = "".concat(link.concat(searchValue)); //Search Data

  (0, _fetchJsonp.default)(toSearch, {
    jsonpCallbackFunction: 'callback'
  }).then(function (res) {
    return res.json();
  }).then(function (data) {
    return displayInfo(data[1], data[3]);
  }).catch(function (err) {
    return console.log(err);
  });
}

var displayInfo = function displayInfo(articleNames, articleLinks) {
  results.innerHTML = "";

  for (var i = 0; i < articleNames.length; i++) {
    var articleName = articleNames[i];
    var articleURL = articleLinks[i]; //Create Anker Tag

    var ankerTag = document.createElement("a");
    ankerTag.innerHTML = "".concat(articleName, " <br>");
